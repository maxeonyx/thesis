...
In order to train a model on large unlabeled data, it needs to be formatted into a pre-training task. The two main types of pre-training task are \textit{masked sequence modeling} and \textit{auto-regressive pretraining}.
...
In the next chapter (\Cref{C:a-o-sampling}) I also experiment with a new pre-training task, which is a variation on auto-regressive sequence modeling, which I call \textit{arbitrary-order autoregressive pretraining}, which I will later apply to hand motion prediction.
